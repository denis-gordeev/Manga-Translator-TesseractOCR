...
def find_contours(
    img_gray, retr=cv2.RETR_TREE, approx=cv2.CHAIN_APPROX_SIMPLE
):
    if int(cv2.__version__[0]) >= 4:
        ...
        im2, contours, hierarchy = cv2.findContours(img_gray, retr, approx)
...
def get_blurbs(img):
    ...
    contours, hierarchy = find_contours(img_gray)
    pruned_contours = []
    ...
    contours2, hierarchy = find_contours(
    ...
    for cnt in contours2:
        ...
        if area > 1000 and area < ((height / 3) * (width / 3)):
            ...
            image = cv2.bitwise_and(
                draw_mask, cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            )
            ...
            if text:
                ...
                print(
                    ...
                    + text
                    + " -> "
                    + translator.translate(text, dest="vi").text
                )
